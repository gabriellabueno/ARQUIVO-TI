...
* _Esquema Externo:_ Visão do usuário
* _Esquema Conceitual:_ Visão conceitual
* _Esquema Interno:_ Visão interna
...
* _Filtragem de dados:_ Operações otimizadas trazem poucos dados, operando diretamente no servidor
...
* _Atomicidade:_ Transações são, geralmente, compostas de várias declarações (comandos / operações), a atomicidade garante que cada transação seja tratada como uma entidade única; se uma única operação do bloco falhar, toda a transação deverá ser cancelada
* _Consistência:_ Assegurar que uma transação somente leve o banco de dados de um estado válido a outro, mantendo a estabilidade do banco
* _Isolamento:_ As transações são isoladas para que as simultâneas não interfiram ou afetem umas às outras; cada solicitação é tratada como se estivesse ocorrendo de forma independente, mesmo que ocorram simultaneamente
* _Durabilidade:_ Garante que as alterações de dados feitas por transações executadas com sucesso sejam preservadas, mesmo em caso de falha do sistema.

### Modelagem de Dados

* Organizar a o conhecimento sobre os dados; construção de um _modelo de dados_ consistente para ser aplicado em qualquer SGBD moderno
...
* _Projeto Conceitual:_ Visão do usuário; entidades, atributos e relacionamentos
  * MER (Modelo Entidade-Relacionamento)
  * DER (Diagrama Entidade-Relacionamento)
* _Projeto Lógico:_ Voltado para implementação; requisitos + modelo de BD
  * Modelo relacional (tabelas)
  * Modelo hierárquico e XML (árvore)
  * Modelo orientado a objetos (classes e objetos complexos)
* _Projeto Físico:_ implementação, codificação
...
* _Domínios básicos:_ inteiro, string
* _Domínios compostos:_ data, hora
* _Domínios definidos:_ (0, 120), (‘M’, ‘F’)
...
* _Cabeçalho:_ Número fixo de atributos (grau da relação)
* _Corpo:_ Número variável de tuplas (cardinalidade da relação)

**Chave:** Identificadores dos dados; pode ser valores/campos combinados (chave composta)

* _Chave primária (pk):_ Identificação única; $$pk (R)$$
* _Chave estrangeira (fk):_ Equivalência de valor com chave primária; $$fk(R_1) \rightarrow pk(R_2)$$
...
* Técnica de _design de banco de dados_ que reduz a redundância de dados e elimina anomalias
...
* _Inclusão:_ Repetição desnecessária de valores ao inserir um dado
* _Atualização:_ Inconsistências no bd ao excluir um dado
* _Exclusão:_ Inconsistências no bd ao atualizar um dado

**Dependências**

* _Funcional :_ Relação entre atributos/colunas
  * ex. `CPF | Nome`
* _Funcional Parcial:_ Coluna não depende da PK ou de toda PK (se for composta)
  * ex. PK = `ID_Aluno` + `ID_Materia` + `Semestre` | `Nome_Materia` | `Nota`
  * Nome\_Materia depende apenas de ID\_Materia
* _Funcional Transitiva:_ Coluna depende de outra coluna que não é uma PK
  * ex. PK = `ID_Funcionario` | `Nome_Funcionario` | `ID_Cargo` | `Nome_Cargo` | `Salario`
  * Nome\_Cargo depende de ID\_Cargo

**Atributos**

* _Multivalorados:_ Podem conter mais de um valor para o mesmo registro (linha)
  * ex. campo Telefone contem dois números, fixo e celular
* _Compostos:_ Atributos que poderiam ser subdivididos em vários atributos
...
* Para números que sofrerão cálculos consecutivos, podem haver arredondamentos que geram _imprecisão no resultado final_
...
*   `ALTER`: Modificar estrutura; adicionar, alterar, excluir colunas; modificar restrições, índices; renomear tabela
    ...
    * `MODIFY`: Modificar atributos de colunas em uma tabela existente, como o tipo de dado ou tamanho ㅤㅤㅤ
...
* Interagem com _dados_ dentro de tabelas; adicionar, editar ou excluir dados de um banco de dados
...
* `TRIGGERS`: gatilho p/ evento especial (pode afetar outras tabelas) ㅤㅤㅤ
