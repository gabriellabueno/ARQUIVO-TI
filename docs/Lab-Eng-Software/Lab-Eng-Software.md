...
**Links**  
- [Spring Annotation Cheat Sheet](https://github.com/Buzzardo/spring-docs/blob/master/annotation-cheat-sheet.adoc)  
- [REQRES | API fake data](https://reqres.in/) 
...
**Stateless:**  A cada nova requisição é recebido infos necessárias p/ funcionalidade pedida por cliente
**Statefull:** Estado de cada cliente é mantido no servidor; cliente fica logado
...
**Mapeamento Objeto-Relacional (ORM):** Ferramentas para auxiliar desenvolvimento de Banco de Dados
**Hibernate:** Ferramenta ORM open sourcem líder de mercado, sendo a inspiração para a especificação Java Persistence API (JPA)
...
**@Repository**
 camada de persistência. Normalmente
anotamos classes que representam um DAO, Repositório
etc.
#### Controller
...
> Para exibir os dados da API em HTML/CSS usando Java no backend (sem JavaScript no frontend), pode-se utilizar a abordagem Server-Side Rendering (SSR) com uma engine de templates, como o **Thymeleaf**.
> Essa abordagem renderiza o HTML diretamente no servidor, incorporando os dados do banco antes de enviar a página ao navegador.
> 
> - *Server-Side Rendering (SSR)* = Renderização das páginas no lado do servidor (geralmente é feita no lado do cliente/navegador)
> - *Engine de templates* = Mecanismo de templates; ferramenta que permite criar páginas HTML dinâmicas de forma estruturada, combinando dados do backend (como variáveis, listas ou objetos) com elementos estáticos (HTML, CSS)

## Como funciona?

- **Thymeleaf:** Processa o template HTML no servidor, substituindo as expressões ( `${brinquedos}`,` th:each`, etc.) pelos dados reais.
- **Fluxo:**
   1. O usuário acessa `http://localhost:8080/brinquedos`
   2. O controller `BrinquedoViewController` busca os alunos via `BrinquedoService`
   3. Os dados são injetados no HTML pelo Thymeleaf
   4. A página é enviada ao navegador já com os dados